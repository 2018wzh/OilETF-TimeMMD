...
def build(cfg_path: Path, incremental: bool = False, skip_visuals: bool = False) -> Dict[str, Any]:
    ...
    if cfg["collection"].get("include_news", True):
        news, news_miss = collect_news.fetch_news(
            cfg["symbols"],
            start_collect,
            end_str,
            cfg["collection"]["news_per_symbol_limit"],
            cfg["collection"],
        )
        news["published_at_utc"] = pd.to_datetime(news["published_at_utc"], utc=True, errors="coerce").astype(str)
        if (incremental or cfg["collection"].get("preserve_existing_news", True)) and raw_news_path.exists():
            raw_news = _concat_dedup(news, raw_news_path, ["article_id"])
        else:
            raw_news = news
    else:
